...
int key_control_speed(chrono::steady_clock::time_point start_time) {
	char key;
	while (1) {
		// 남은 시간 갱신
...
bool move_and_place_block(char*** m_board, char*** c_board, char*** block, char*** f_block, char*** s_block, char*** t_block, bool* block_used, int point, int& high_score) {
	...
	bool move_and_place_block(
		char*** m_board, char*** c_board, char*** block,
		char*** f_block, char*** s_block, char*** t_block,
		bool* block_used, int& point, int& high_score,
		...
		while (1) {
			...
			key = key_control_speed(start_time);
			// 제한 시간 초과 시 종료
			if (key == -2) {
				system("cls");
			...
			if (game_num == 1) {
				...
				while (1) {
					...
					if (key == k_1) {
						if (!block_used[0]) {
							block_used[0] = true;
							block_used[0] = move_and_place_block(
								m_board, c_board, f_block, f_block, s_block, t_block,
								block_used, total_point, high_score, start_time);
							if (block_used[0]) {
								total_point += count_block_cells(f_block);
								main_block(f_block);
							}
						...
						if (!block_used[1]) {
							block_used[1] = true;
							block_used[1] = move_and_place_block(
								m_board, c_board, s_block, f_block, s_block, t_block,
								block_used, total_point, high_score, start_time);
							if (block_used[1]) {
								total_point += count_block_cells(s_block);
								main_block(s_block);
						...
						if (!block_used[2]) {
							block_used[2] = true;
							block_used[2] = move_and_place_block(
								m_board, c_board, t_block, f_block, s_block, t_block,
								block_used, total_point, high_score, start_time);
							if (block_used[2]) {
								total_point += count_block_cells(t_block);
								main_block(t_block);
